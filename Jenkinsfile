...
node ('master') {
   stage ('Checkout'){
    ...
    stage ('Docker Build'){
        bat """
        docker build  --rm -t https://hub.docker.com/repositories/arifroid/example-nodejs-api:v1 .
      """
    }
    stage ('Docker Push'){
        bat """
        docker push https://hub.docker.com/repositories/arifroid/example-nodejs-api:v1
      """
    }
